...
1. 列表入场只在**首次挂载/视图切换**播放；WS 增量推送的新项只对该项做单项 240ms 淡入（不重放 stagger）。
   **240ms 裁定（2026-09-24，v0.0.4 beta 真机回归 §6.4）**：本条原写 200ms，实现恒为 `itemVariants` 的 240ms（`lib/motion.ts:52`，与 §65 列表入场子项同一档）。裁定时比对过两种收口方向——把新项淡入拆成独立 200ms 档要新增一份 variant 并偏离既有同源档；改文档跟实现则零观感代价（240 与 200 均为 §1 允许值，差 40ms 属观感级）。取后者：**规范改 240ms，代码不动**，新项与列表入场继续共用 `itemVariants`。§5.2 回落规则里的「新列单项淡入」同为此档，一并按 240ms 读。
...
- **回落规则（任一命中即退化为「旧列 exit 140ms + 新列单项淡入 240ms」，240 见 §5.1 裁定）**：
