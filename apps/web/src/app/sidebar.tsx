...
export function Sidebar() {
  ...
  return (
    ...
    >
      {/* 品牌：13.1 顶部 [◆] 纯标识；折叠按钮固定在导航最底部（用户拍板 2026-09-23）。 */}
      ...
      <div className={cn('shrink-0 border-t border-border py-2', collapsed ? 'px-0' : 'px-2')}>
        <button
          type="button"
          ...
          className={cn(
            'flex h-11 w-full items-center rounded-control text-nav text-text-secondary transition-colors duration-140 ease-settle hover:bg-bg-muted hover:text-text-primary',
            collapsed ? 'justify-center' : 'gap-3 px-3',
          )}
        >
          {collapsed ? (
            ...
            <>
              ...
              <span className="truncate">收起导航</span>
            </>
          )}
        </button>
      </div>
