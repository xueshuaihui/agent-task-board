...
- 推断用户query对应的意图，调用`call_edu_agent.py` 时显示的告知意图 如: paper_compose
...
- 用于用户学习模拟卷组卷，非查找存在的试卷需求，而是组一套新试卷
- 不在本SKILL的服务范畴内的意图如下，以下意图不应该调用call_edu_agent.py：

  1. **听力/口语类** → 听力、口语类题目不触发组卷。
    - 示例：“来一套听力模拟测试”

  2. **含既有文件的题目导入/整理/提取** → 用户明确提出「导入」「整理」「提取」文件中的题目，且附有既有文件时，不触发组卷。
    - 示例：“这些是我的题目你能全部帮我优化后导入吗，我的程序里面都有划分每一年纪的上下册还有单元，我只需要文件里面的题目@【人教部编版】小学语文1-6年级试卷”

  3. **非K12范围** → 服务仅限小学、初中、高中。职考、公考、教资、会计等职业资格考试，以及大学、考研、留学考试等均不触发组卷。
    - 示例：“帮我出一套公务员行测模拟卷”

| 字段 | 处理规则 |
|------|----------|
| 学科 | 从 query 推断；缺失可直接调用 |
| 学段 | 从 query 推断；缺失可直接调用 |
| 年级 | 从 query 推断；中考/高考可留空；缺失可直接调用 |
| 地区（省+城市） | 从 query 推断；无法推断可默认大省市（如浙江杭州）或直接调用 |
| 考试类型 | 从 query 推断（期中/期末/月考/中考/高考）；缺失可直接调用 |
| 学期 | 优先按当前日期推断（1-7月→下学期，8-12月→上学期）；中考/高考留空；否则缺失可直接调用 |
...
python /app/skills/edu-agent/scripts/call_edu_agent.py \
  --message "请先调用工具读取paper_compose 在完成下面任务：组一套二元一次函数的单元练习卷 " \
  --session-id "<uuid>"

# 复用 session 实现多轮
python /app/skills/edu-agent/scripts/call_edu_agent.py \
  --message "请先调用工具读取paper_compose 在完成下面任务：能否再组一套试卷" \
  --session-id "<uuid>"
```
