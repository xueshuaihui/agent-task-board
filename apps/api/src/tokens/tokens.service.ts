...
export class TokensService {
   ...
   */
  async list(): Promise<{ items: TokenDto[] }> {
  ...
  async issue(input: TokenCreateInput): Promise<IssuedTokenDto> {
    ...
    return { ...toTokenDto(await this.get(id)), token: plaintext };
  }
   ...
   */
  async revoke(id: string): Promise<{ id: string; enabled: false; revoked: boolean }> {
    await this.get(id);
